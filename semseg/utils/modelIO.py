...
def save_model(model, directory, metadata=None, filename=MODEL_FILENAME):
    ...
    if metadata is None:
        ...
        metadata = dict(img_size=model.img_size, latent_dim=model.num_classes,
...
def load_model(directory, is_gpu=True, filename=MODEL_FILENAME):
    ...
    path_to_model = os.path.join(directory, filename)
    model = _get_model(model_type, img_size, num_classes, device, path_to_model)
    return model
...
def _get_model(model_type, img_size, num_classes, device, path_to_model):
    """ Load a single model.
    Parameters
    ----------
    model_type : str
    ...
    """
    model = init_specific_model(model_type, img_size, num_classes).to(device)
